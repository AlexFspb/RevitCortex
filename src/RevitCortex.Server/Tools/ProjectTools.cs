...
public static class ProjectTools
{
    ...
    [McpServerTool(Name = "send_code_to_revit"), Description("LAST RESORT ONLY — execute custom C# code in Revit. Prefer dedicated tools whenever they cover the task. Parameter edits -> set_element_parameters / bulk_modify_parameter_values; queries & filtering -> ai_element_filter / filter_by_parameter_value / export_elements_data; model stats -> analyze_model_statistics / check_model_health; deletion -> delete_element; transforms -> modify_element; views, schedules, rebar and steel -> their dedicated tools. Use this ONLY when no dedicated tool covers the operation (e.g. exotic geometry creation, read-only inspection of an uncovered Revit API, or a one-off operation no dedicated tool covers) — never for modal family editing (Document.EditFamily deadlocks from the tool's external-event context) — within the user-authorized task; no separate chat approval is required. For every script-owned mutation transaction in group/none, call RevitCortex.Tools.CodeExecution.ScriptFailureHandling.Configure(tx) after Start and before changes. Auto mode configures it automatically. Errors roll back; warnings are removed from the Revit failure dialog and returned with descriptions and element IDs; inspect descriptions, severity and element IDs, verify commit status and retain the diagnostic report. Use a bounded dry-run before bulk replacement. Never force-accept errors or delete affected elements as recovery. Return plain data; ElementId is converted to its 64-bit Value, but other raw Revit API objects are rejected. Arrays and lazy LINQ are supported within limits: depth 16, 100000 nodes, 256 KiB per string, 1 MiB response. ResultSerializationFailed includes the result path and transactionState: auto/group attempt rollback before commit, none cannot guarantee rollback. Do not blindly retry; verify state and external effects first. Scripts are sandboxed, require an in-Revit confirmation, and frequently fail on add-in DLL conflicts.")]
